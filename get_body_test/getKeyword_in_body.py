...
# TextRank: gensim의 keywords 함수를 활용 (텍스트 요약 기반)
from gensim.summarization import keywords as gensim_keywords

# Word2Vec, FastText 관련 라이브러리
...
def extract_keywords_tfidf(text):
    """
    TF-IDF를 이용하여 단일 문서(텍스트) 내에서 키워드 상위 5개를 추출합니다.
    (단일 문서에서는 idf가 모두 동일하게 나오므로, 실질적으로는 단순 빈도 기반의 효과를 가집니다.)
    """
    vectorizer = TfidfVectorizer()
    tfidf_matrix = vectorizer.fit_transform([text])
    feature_names = vectorizer.get_feature_names_out()
    # tfidf 점수는 한 문서 내에서 정규화된 값으로 계산됩니다.
    scores = tfidf_matrix.toarray()[0]
    # 점수가 높은 순으로 정렬하여 상위 5개 단어 추출
...
def extract_keywords_rake(text):
    """
    RAKE는 불용어와 구두점을 기준으로 문장을 분할하여 후보 구를 생성하고, 각 후보에 점수를 부여합니다.
    여기서는 상위 5개의 구를 반환합니다.
    """
    r = Rake()  # 기본 불용어 사전 사용
...
def extract_keywords_textrank(text):
    """
    gensim의 keywords 함수를 사용하여 TextRank 기반으로 키워드를 추출합니다.
    lemmatize=True 옵션을 통해 단어 원형을 고려합니다.
    """
    try:
        keywords_list = gensim_keywords(text, words=5, split=True, lemmatize=True)
...
def extract_keywords_word2vec(text):
    ...
    각 단어의 중요도를 산정하는 방식으로 키워드를 추출합니다.
    
    (여기서는 단순한 예제로 문장을 '.' 기준으로 분리하여 학습합니다.)
    """
    # 간단한 문장 분리 및 토큰화 (더 정교한 전처리가 필요할 수 있음)
    ...
    vocab = model.wv.index_to_key  # gensim 4.x 기준
    # 문서 전체의 벡터를 모든 단어 벡터의 평균으로 계산
...
def extract_keywords_fasttext(text):
    ...
    Word2Vec과 동일한 방식으로 전체 문서 벡터와의 유사도를 계산해 상위 5개 단어를 반환합니다.
    """
    sentences = [sentence.split() for sentence in text.split('.') if sentence.strip()]
    if not sentences:
        return []
...
def extract_keywords_yake(text):
    """
    YAKE는 단일 문서 내 통계적 특징(단어 위치, 주변 단어 등)을 고려하여 키워드를 추출합니다.
    여기서는 언어 옵션을 "en"으로 지정했으나, 필요에 따라 "ko" 등으로 변경할 수 있습니다.
    """
    # n=1: 단일 단어, top=5: 상위 5개 키워드
    kw_extractor = yake.KeywordExtractor(lan="en", n=1, top=5)
    keywords = kw_extractor.extract_keywords(text)
    # YAKE는 (키워드, 점수) 튜플을 반환하므로, 점수가 낮을수록 좋은 키워드입니다.
...
def extract_keywords():
    """
    POST 요청 시 JSON 본문에서 "text" 필드에 해당하는 크롤링 텍스트를 받아,
    6가지 방법으로 각각 중요 키워드 5개씩을 추출하여 JSON 형태로 반환합니다.
    """
